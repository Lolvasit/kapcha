...
users_verifying = dict() # словарь чтобы запомнить юзеров которые нажали на запрос
verification_spam_time = 30;
capcha_message = "Подтвердите что вы человек"
verification_message = "Готово"
...
async def join_request(message: types.Message):

    markup = ReplyKeyboardMarkup(resize_keyboard=True)
    markup.add(KeyboardButton("Подтвердить"))

    message_data = await bot.send_message(message.from_user.id, capcha_message, reply_markup=markup)
    ...
    await resend_capcha(message_data, user_id)
    # await bot.approve_chat_join_request(
    #      message.chat.id,
    #      message.from_user.id)

# @dp.callback_query_handler(text="apply")
# async def _apply(callback: CallbackQuery):
...
#
#     # await bot.approve_chat_join_request(
#     #       users_verifying[user_id],
#     #       user_id)
#
#     await bot.send_message(callback.from_user.id, "Готово")

@dp.message_handler(text="Подтвердить")
async def _apply(message: Message):
    user_id = message.from_user.id
    ...
    await bot.send_message(message.from_user.id, verification_message)


async def resend_capcha(message_data, user_id):
    await asyncio.sleep(verification_spam_time)
    try:
        await bot.delete_message(message_data.chat.id, message_data.message_id)
    except:
        return
    if user_id in users_verifying:
        message_data = await bot.send_message(message_data.chat.id, message_data.text, reply_markup=get_capcha_markup())
        await resend_capcha(message_data, user_id)
...
@dp.message_handler(commands=["verifset"])
async def _verification_set(message: types.Message, state: FSMContext):
    await VerificationSet.capcha_msg.set()
    await message.answer("Введите сообщение капчи")

@dp.message_handler( state=VerificationSet.capcha_msg, content_types=ContentType.ANY)
async def _verification_set(message: types.Message, state: FSMContext):
    global capcha_message
    capcha_message = message.text
    await VerificationSet.msg.set()
    await message.answer("Введите сообщение после капчи")

@dp.message_handler( state=VerificationSet.msg, content_types=ContentType.ANY)
async def _verification_set(message: types.Message, state: FSMContext):
    global verification_message
    verification_message = message.text
    await VerificationSet.resend_time.set()
    await message.answer("Введите время повтора капчи")


@dp.message_handler( state=VerificationSet.resend_time, content_types=ContentType.ANY)
async def _verification_set(message: types.Message, state: FSMContext):
    try:
        global verification_spam_time
        verification_spam_time = int(message.text)
    except:
